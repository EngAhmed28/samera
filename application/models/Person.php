...
//-----------------------------------
  ...
  public function select_data(){
     ...
     public  function  insert($file_name,$date){
...
//die;
        ...
        $data['house_state'] = $this->input->post('house_state');
        $data['house_type'] = $this->input->post('house_type');
        if($this->input->post('house_type') == 7){
            $data['house_owner'] = $this->input->post('house_owner');
            $data['house_rent'] = $this->input->post('house_rent');
        }
        $data['card_num'] = $this->input->post('card_num');
        $data['card_date'] = $this->input->post('card_date');
        $data['card_source'] = $this->input->post('card_source');
        $data['male_num'] = $this->input->post('male_num');
        $data['femal_num'] = $this->input->post('femal_num');
        ...
        //$data['other'] = $this->input->post('other_person');
        $data['social_status'] = $this->input->post('social_status');
        if($this->input->post('social_status') == 16){
            $data['orphan_name'] = $this->input->post('orphan_name');
            $data['orphan_propety'] = $this->input->post('orphan_propety');
            $data['orphan_propety_rent'] = $this->input->post('orphan_propety_rent');
            $data['orphan_prove'] = $file_name;
        }
        $data['health_state'] = $this->input->post('health_state');
        $data['house_state'] = $this->input->post('house_state'); 
        $data['approved'] = 0; 
   ...
   public function getById($id){
        ...
        return array($h1->row_array(),$h2->result(),$h3->row_array());
   } 
